...
public class ApplicationLog extends Activity {
		...
		for (ApplicationInfo applicationInfo : apps) {
			...
			if (currentAppName.equals(applicationInfo.getProcessName())) {
				...
				if (currentApp.getLastToTime() != null
					...
					currentApp.addToTíme(applicationInfo.getIndividualTime());
				} else {
					currentApp.setLastToTíme(applicationInfo
							.getIndividualTime());
				}
			} else {
				...
				if (found) {
					if (currentApp.getLastToTime() != null
						...
						currentApp.addFromTime(applicationInfo
								.getIndividualTime());
						currentApp.addToTíme(applicationInfo
								.getIndividualTime());
					} else {
						currentApp.setLastToTíme(applicationInfo
								.getIndividualTime());
	...
	class ApplicationInfo implements Comparable<ApplicationInfo> {
		...
		public void setLastToTíme(Date individualTime) {
			_dateTo.set(_dateTo.size() - 1, individualTime);

		}

		public void addToTíme(Date individualTime) {
